...
- Keep material authoring simple for common behavior.
- Expose one step API with selectable backend mode.
- Avoid hard dependency on any specific threading framework.
...
typedef enum rg_status_e {
    RG_STATUS_OK = 0,
    RG_STATUS_INVALID_ARGUMENT,
    RG_STATUS_NOT_FOUND,
    RG_STATUS_ALREADY_EXISTS,
    RG_STATUS_CAPACITY_REACHED,
    RG_STATUS_ALLOCATION_FAILED,
    RG_STATUS_OUT_OF_BOUNDS,
    RG_STATUS_CONFLICT,
    RG_STATUS_UNSUPPORTED
} rg_status_t;
...
## Allocator and Runner Abstractions
...
typedef struct rg_runner_vtable_s {
    rg_runner_parallel_for_fn parallel_for; /* synchronous */
    rg_runner_worker_count_fn worker_count; /* optional, can return 1 */
} rg_runner_vtable_t;
...
typedef struct rg_world_config_s {
    int32_t chunk_width;   /* default: 64 */
    int32_t chunk_height;  /* default: 64 */
    uint16_t inline_payload_bytes; /* default: 16 */
    uint16_t max_materials;        /* default: 256 */
    ...
    const rg_runner_t* runner; /* nullable: serial fallback */
} rg_world_config_t;
...
typedef struct rg_material_desc_s {
    ...
    rg_material_update_fn update_fn; /* optional */
    void* user_data;
} rg_material_desc_t;
...
typedef struct rg_cell_read_s {
    rg_material_id_t material_id;
    const void* instance_data; /* read-only pointer, nullable */
} rg_cell_read_t;

typedef struct rg_cell_write_s {
    rg_material_id_t material_id;
    const void* instance_data; /* nullable for default init */
} rg_cell_write_t;
...
## Update Context Helpers (for material callbacks)
...
- Whether default material movement presets should be first-class helpers.
- Whether callback API should expose neighborhood snapshots directly.
- Whether chunk load/unload should support user-provided serialization hooks.
